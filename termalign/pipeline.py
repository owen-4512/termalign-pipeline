...
from .io_utils import SentencePair, read_dictionary, read_sentence_pairs, write_csv
...
def run_pipeline(
    ...
    dict_zh = read_dictionary(dict_zh_path)
    dict_en = read_dictionary(dict_en_path)

    zh_terms = extract_terms(pairs, dict_zh, bert_model_zh, "zh", skip_bert)
    en_terms = extract_terms(pairs, dict_en, bert_model_en, "en", skip_bert)

    write_csv(
        output_path / "terms_zh.csv",
        [
            {
                "term": term.term,
                "source": term.source,
    ...
    write_csv(
        output_path / "alignments.csv",
        [
            {
                "zh_term": alignment.zh_term.term,
                ...
                "zh_sentence": alignment.zh_term.sentence,
