...
class Embedder:
    def __init__(self, model_name_or_path: str, device: str | None = None, batch_size: int = 32) -> None:
        if device is None:
            ...
            if isinstance(modules, list):
                for item in modules:
                    if not isinstance(item, dict):
                        continue
                    if "Transformer" not in str(item.get("type", "")):
                        continue
    ...
    def encode(self, texts: Iterable[str]) -> np.ndarray:
        ...
        if self._use_sentence_transformer and self.sentence_transformer is not None:
            vectors = self.sentence_transformer.encode(texts_list, convert_to_numpy=True, show_progress_bar=False)
            return np.asarray(vectors)
        if self._use_hash_fallback:
            return self._encode_hash(texts_list)
...
def align_terms(zh_terms: List[TermOccurrence], en_terms: List[TermOccurrence], embedder: Embedder) -> List[AlignmentResult]:
    ...
    for idx, zh_term in enumerate(zh_terms):
        ...
        results.append(AlignmentResult(zh_term=zh_term, en_term=en_terms[best_idx], similarity=float(similarity_matrix[idx, best_idx])))
    return results
