...
from .data_model import DistanceInputs, build_extracted_records_from_tsv, build_gold_map, build_token_map_batch, build_token_map_simple, collapse_records_for_simple_mode
...
def _compute_metric_bundle(records: List[Mapping[str, Any]], gold_map: Mapping[str, set[str]], token_map: Mapping[str, DistanceInputs], selected_metrics: List[str], alpha: float, beta: float) -> Dict[str, Any]:
    del token_map, beta
    result: Dict[str, Any] = {}
    precision = consistency = 0.0
...
def run_evaluation(term_align_tsv: Path, gold_jsonl: Path, mode: str, metrics: Iterable[str], alpha: float, beta: float, target_txt: Path | None = None, target_dir: Path | None = None, report_level: str = "batch", include_debug: bool = False, include_cross_document_consistency: bool = False) -> Dict[str, Any]:
    ...
    if report_level in {"document", "both"}:
        result["document_scores"] = [{"source_file": str(record.get("source_file", "__default__")), **_compute_metric_bundle([record], gold_map, _single_record_token_map(record, token_map), selected_metrics, alpha, beta)} for record in records]
