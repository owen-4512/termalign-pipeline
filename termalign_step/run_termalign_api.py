...
def _resolve_default_output_file(task1_input_dir: str | None, pipeline_run: bool, evaluation_pipeline_root: str | None) -> Path:
...
def _request_one(session: Any, api_endpoint: str, headers: dict[str, str], row: dict[str, Any], min_pair_confidence: float, api_model: str | None, prompt_text: str | None, timeout: int) -> tuple[str, list[dict[str, Any]]]:
    ...
    response = session.post(api_endpoint, headers=headers, json=payload, timeout=timeout)
    ...
    for pair in pairs:
        ...
        out_rows.append(_to_std_row(item, source_file, source_segment, target_segment))
    return source_file, out_rows
...
def run_termalign_api(
    ...
    with requests.Session() as session:
        with ThreadPoolExecutor(max_workers=max(1, workers)) as ex:
            ...
            for fut in as_completed(futures):
                source_file, out_rows = fut.result()
                if out_rows:
                    per_file.setdefault(source_file, []).extend(out_rows)

    all_rows: list[dict[str, Any]] = []
    all_high_rows: list[dict[str, Any]] = []

    blank_zh = {k: "" for k in TSV_FIELDS}
    blank_en = {k: "" for k in TSV_FIELDS}
    ...
    for source_file, file_rows in per_file.items():
        ...
        zh_terms = sorted({r["zh_term"] for r in file_rows if r.get("zh_term")})
        en_terms = sorted({r["en_term"] for r in file_rows if r.get("en_term")})
        _write_tsv(details_dir / f"{source_base}_terms.zh.tsv", [{**blank_zh, "zh_term": t} for t in zh_terms])
        _write_tsv(details_dir / f"{source_base}_terms.en.tsv", [{**blank_en, "en_term": t} for t in en_terms])

        all_rows.extend(file_rows)

    all_rows.sort(key=lambda x: float(x["similarity"]), reverse=True)
    _write_tsv(details_dir / "all_alignments.tsv", all_rows)
    _write_tsv(details_dir / "all_terms.zh.tsv", [{**blank_zh, "zh_term": t} for t in sorted({r["zh_term"] for r in all_rows if r.get("zh_term")})])
    _write_tsv(details_dir / "all_terms.en.tsv", [{**blank_en, "en_term": t} for t in sorted({r["en_term"] for r in all_rows if r.get("en_term")})])
