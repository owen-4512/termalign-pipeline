...
HF_CACHE_ROOT = Path.home() / ".cache" / "termalign_hf_models"
...
def _model_local_dir(repo_id: str) -> Path:
    return HF_CACHE_ROOT / repo_id.replace("/", "--")


def _looks_downloaded(model_dir: Path) -> bool:
    if not model_dir.exists() or not model_dir.is_dir():
        return False
    marker_files = ("config.json", "tokenizer_config.json", "modules.json", "model.safetensors", "pytorch_model.bin")
    return any((model_dir / m).exists() for m in marker_files)


def _download_one_model(repo_id: str, local_only: bool) -> str:
    from huggingface_hub import snapshot_download

    local_dir = _model_local_dir(repo_id)
...
def _ensure_hf_models_downloaded() -> tuple[str, str, str]:
    ...
    repo_ids = (HF_ZH_MODEL, HF_EN_MODEL, HF_ALIGN_MODEL)
    resolved = {rid: rid for rid in repo_ids}

    with ThreadPoolExecutor(max_workers=3) as ex:
        future_map = {ex.submit(_download_one_model, rid, local_only): rid for rid in repo_ids}
        for fut in as_completed(future_map):
            rid = future_map[fut]
            try:
                resolved[rid] = fut.result()
            except Exception:
                # fallback to repo id for online lazy download by downstream libs
                resolved[rid] = rid
...
def _jsonl_to_tsv(input_jsonl: Path, output_tsv: Path) -> str:
    ...
    with input_jsonl.open("r", encoding="utf-8") as fin, output_tsv.open("w", encoding="utf-8", newline="") as fout:
        writer = csv.DictWriter(fout, fieldnames=["src_text", "tgt_text"], delimiter="\t")
        writer.writeheader()
        for line in fin:
            if not line.strip():
                continue
            row = json.loads(line)
            writer.writerow({"src_text": row.get("source_segment", ""), "tgt_text": row.get("target_segment", "")})
...
def _organize_alignment_outputs(source_dir: Path, details_dir: Path, min_pair_confidence: float) -> tuple[Path | None, Path | None]:
    ...
    generated_names = {"all_alignments.tsv", "all_alignments_high_conf.tsv"}
    all_rows = []
    high_rows = []
    per_file_outputs = 0
    ...
    for f in source_dir.rglob("*.tsv"):
        ...
        try:
            ...
            continue
        if df.empty or "zh_term" not in df.columns or "en_term" not in df.columns:
            continue
...
def run_termalign(
    ...
    del extraction_mode, min_term_confidence, source_lang, target_lang, top_k_pairs, api_endpoint, api_key, device, api_model, api_prompt_file
